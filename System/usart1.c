...
{
	/*开启时钟*/
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_USART1, ENABLE);	//开启USART1的时钟（语音识别）
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);					//将PB10引脚初始化为复用推挽输出,PB10是TX
	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;
	GPIO_Init(GPIOA, &GPIO_InitStructure);					//将PB11引脚初始化为输入,PB11是RX
